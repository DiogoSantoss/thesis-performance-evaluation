...
# List of file names
file_names = [  
    'data/temp_cluster_qbft.json',
    'data/temp_cluster_alea.json'
...
for i in range(len(file_names)):
    plt.errorbar(load, latency_avg[i], yerr=latency_std_devs[i], label=file_names[i].split('/')[1].split('.')[0].split('_')[-1], fmt='-o', capsize=5)

# Adding labels and legend
plt.xlabel('Peer Network Latency (ms)')
plt.ylabel('Latency (ms)')
plt.title('Latency with increasing latency between peers')
plt.legend()
